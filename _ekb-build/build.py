...
def compose_geo(product_key, city_key):
    ...
    city_q = (f'Сколько стоит доставка {city["to"]}?',
              f'{hint} Точную цену за куб и за мешок с доставкой называем в ответ на заявку под ваш объём и адрес.')
...
def render_hub(all_pages):
    ...
    about = [
        ...
        "Объём считается одинаково для любого материала: площадь умножить на толщину слоя. Куб на сотку даёт слой в один сантиметр, значит сотка под слой 10 сантиметров это 10 кубов, теплица три на шесть при замене 20 сантиметров это 3,6 куба. К расчёту добавляют примерно пятую часть на усадку: свежая насыпь садится после первого полива. Посчитать можно калькулятором выше или сказать нам размеры, посчитаем сами.",
        "Возим и мешками, и навалом. Мешок 40-50 литров, в кубе их 20-25, минимальный заказ 3 куба, то есть 60-75 мешков. Фасовку берут не ради цены, за куб она всегда дороже, а ради подъезда: когда самосвалу негде встать или материал надо занести руками к конкретной грядке. Если подъезд есть и объём больше пяти кубов, навалом заметно дешевле за кубометр.",
        "Работаем по Екатеринбургу и области примерно до 100 километров. По городу и ближнему пригороду чаще всего успеваем в день заказа, в дальние города планируем на ближайшие дни. Цену за куб и за мешок с доставкой в ваш район называем сразу по заявке, оплата после выгрузки.",
    ]
    faq = [
        ...
        ("Как узнать цену?", "Назовите продукт, объём и адрес в заявке или в мессенджере, назовём точную цену за куб и за мешок с доставкой в ваш район. Скрытых доплат нет."),
        ...
        ("Какой минимальный заказ?", "Три кубометра, это 60-75 мешков. Меньше не возим: машина всё равно выезжает и грузится, и рейс на десять мешков не окупается ни нам, ни вам."),
        ("Что дешевле, мешки или машина?", "За кубометр машина всегда дешевле: фасовку насыпают и грузят руками. Мешки берут, когда самосвалу не подъехать или материал надо занести к конкретной грядке. От пяти кубов навалом выгоднее однозначно."),
