...
NOINDEX = {
    ...
    # Реальное плечо доставки 60-80 км от Екатеринбурга. Тагил (140 км) и
    # Каменск-Уральский (100 км) за его пределами: собирать по ним заявки
    # значит обещать то, чего мы не выполним.
    "peregnoy-nizhniy-tagil", "chernozem-kamensk-uralskiy",
...
def attach_related(pages):
    ...
    Страницы вне зоны доставки в перелинковку не попадают: незачем предлагать
    посетителю город, куда мы не возим."""
    by_slug = {p["slug"]: p for p in pages}
    pages_ok = [q for q in pages if q["slug"] not in NOINDEX]
    for p in pages:
        rel = []
        prod = p.get("product")
        city_key = p.get("city")
...
def render_hub(all_pages):
    ...
    faq = [
        ("Какие города вы обслуживаете?", "Возим по Екатеринбургу и ближней области, это примерно 80 км от города: Первоуральск, Ревда, Дегтярск, Сысерть, Полевской, Белоярский, Заречный, Арамиль, Берёзовский и посёлки между ними. Дальше стоимость доставки перевешивает стоимость самого грунта, поэтому в дальние районы области не возим."),
