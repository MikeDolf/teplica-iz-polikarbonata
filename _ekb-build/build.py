...
def min_volume_note(city_key=None):
    ...
    if min_m3(city_key) == 1:
        return "плечо до вас короткое, а на такой объём подаём небольшую машину до 3 м³ — она проедет в любой двор"
...
def render_hub(all_pages):
    ...
    faq = [
        ...
        ("В каком объёме возите?", "Только навалом, самосвалом, от трёх кубов. Самосвал 5 т берёт 3-7 кубов, КамАЗ до 10 кубов, самосвал с наращенными бортами до 26 кубов за рейс. Мешками и меньшим объёмом не возим: поставщика по фасовке у нас нет, а рейс под полкуба не окупается. Подскажем, какая машина пройдёт к вашему участку."),
