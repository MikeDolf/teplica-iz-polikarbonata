...
def bag_note(product_key):
    ...
    if not SITE.get("bags"):
        return "это кузов малого самосвала"
...
def render_hub(all_pages):
    ...
    faq = [
        ...
        ("В каком объёме возите?", "Только навалом, самосвалом, от трёх кубов. Малый самосвал берёт 3-4 куба, средний 5-7, КамАЗ от 10 и выше. Мешками и меньшим объёмом не возим: поставщика по фасовке у нас нет, а рейс под полкуба не окупается. Подскажем, какая машина пройдёт к вашему участку."),
